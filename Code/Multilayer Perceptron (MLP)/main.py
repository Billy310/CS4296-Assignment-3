...
train_data = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load(argparse.train_csv_path)
...
test_data = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load(argparse.test_csv_path)
test_data = assembler.transform(test_data)

result = model.transform(test_data)
predictionAndLabels = result.select("prediction", "label")

predictionAndLabels.write.format("csv").option("header", "true").save(argparse.predictions_path)
...
train_results.saveAsTextFile(argparse.results_path)